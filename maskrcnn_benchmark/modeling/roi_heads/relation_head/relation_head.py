...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
            output_losses = dict(loss_rel=loss_relation[0], loss_refine_obj=loss_refine[0],
                                 loss_refine_att=loss_refine[1])
        else:
            output_losses = dict(loss_rel=loss_relation[0], loss_refine_obj=loss_refine)

        if self.use_loss:
            loss_relation_dict = dict(loss_relation_contrastive=loss_relation[1])
        else:
            loss_relation_dict = dict()
        output_losses.update(add_losses)

        return roi_features, proposals, output_losses, loss_relation_dict
        ...
        # output_losses['rel_loss'] = self.criterion_loss(relation_logits, rel_labels)
        # output_losses.update(add_losses)
