...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            # this makes the API consistent during training and testing
            if self.training:
                # x, detections, loss_relation, loss_relation_tuple = self.relation(features, detections, targets, logger,iteration,cm)
                x, detections, loss_relation, loss_relation_tuple = self.relation(features, detections, targets, logger)
                losses.update(loss_relation)
                return x, detections, losses, loss_relation_tuple
            else:
                x, detections, loss_relation = self.relation(features, detections, targets, logger)
                losses.update(loss_relation)
                return x, detections, losses
